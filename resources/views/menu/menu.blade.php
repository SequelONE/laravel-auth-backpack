    ...
    @if($item->status === 1)
        <li class="nav-item">
            @if ($item->children->count() === 0)
                <a href="{{ $item->url() }}" class="nav-link{{ (url()->current() === $item->url()) ? ' active ' : '' }}" aria-current="page">{{ $item->title }}</a>
            @endif
            @if ($item->children->count() > 0)
                <div class="btn-group">
                    <a href="{{ $item->url() }}" class="btn @if(url()->current() === $item->url()) btn-primary active @endif " aria-current="page">{{ $item->title }}</a>
